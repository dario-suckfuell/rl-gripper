...
class CustomActor(BasePolicy):
    """
    CustomActor network (policy) for SAC.
    ...
    def __init__(
        ...
        latent_pi_net = create_simba_mlp(features_dim, -1, net_arch, activation_fn)
...
MlpPolicy = CustomSACPolicy


class CnnPolicy(CustomSACPolicy):
    """
    Policy class (with both actor and critic) for SAC.
...
class MultiInputPolicy(CustomSACPolicy):
    """
    Policy class (with both actor and critic) for SAC.
...
class LatentPiNetwork(nn.Module):
    def __init__(self, features_dim: int = 512, num_heads: int = 8):
        super(LatentPiNetwork, self).__init__()
